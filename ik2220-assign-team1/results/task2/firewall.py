...
fw1_fwd_rules = [ ('ICMP', '100.0.0.50', '*', '100.0.0.10', '*'),
                                  ('ICMP', '100.0.0.50', '*', '100.0.0.11', '*'),
                                  ('ICMP', '100.0.0.51', '*', '100.0.0.10', '*'),
                                  ('ICMP', '100.0.0.51', '*', '100.0.0.11', '*'),
                                  ('ICMP', '100.0.0.10', '*', '100.0.0.50', '*'),
                                  ('ICMP', '100.0.0.11', '*', '100.0.0.50', '*'),
                                  ('ICMP', '100.0.0.10', '*', '100.0.0.51', '*'),
                                  ('ICMP', '100.0.0.11', '*', '100.0.0.51', '*'),
                                  ('TCP', '100.0.0.50', '*', '100.0.0.10', '1025'),
                                  ('TCP', '100.0.0.51', '*', '100.0.0.10', '1025'),
                                  ('TCP', '100.0.0.50', '*', '100.0.0.11', '1025'),
                                  ('TCP', '100.0.0.51', '*', '100.0.0.11', '1025'),
                                  ('TCP', '100.0.0.10', '1025', '100.0.0.50', '*'),
                                  ('TCP', '100.0.0.11', '1025', '100.0.0.50', '*'),
                                  ('TCP', '100.0.0.10', '1025', '100.0.0.51', '*'),
                                  ('TCP', '100.0.0.11', '1025', '100.0.0.51', '*')]
...
class firewall (object):
  ...
  def _handle_PacketIn (self, event):
    ...
    # Checks the incoming packet against the firewall rules of FW1 (near Private zone)
    def runFirewall1():
        ...
        if icmp is not None:
                ...
                if icmp.type == 0:
                        log.debug("Firewall 1 - ICMP ECHO REPLY")
                        for (protocol,srcIP,srcPort,dstIP,dstPort) in fw1_fwd_rules:
                                if(protocol == 'ICMP'):
                                        if(str(src_ip_addr) == srcIP and str(dst_ip_addr) == dstIP):
                                                return True
                if icmp.type == 8:
                        log.debug("Firewall 1 - ICMP ECHO REQUEST")
                        for (protocol,srcIP,srcPort,dstIP,dstPort) in fw1_fwd_rules:
                                if(str(src_ip_addr) == '100.0.0.50' or str(src_ip_addr) == '100.0.0.51'):
                                        return True
                                        ...
                                        if(str(src_ip_addr) == srcIP and str(dst_ip_addr) == dstIP):
                                                if(srcPort == '*'):
                                                        if(dstPort == '*'):
                                                                ...
                                                                if(str(src_ip_addr) != '100.0.0.50' or str(src_ip_addr) != '100.0.0.51' and packet.find('tcp').ack == 0):
